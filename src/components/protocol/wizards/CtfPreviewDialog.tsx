...
export default function CtfPreviewDialog({
  ...
  const [isWaitingPreview, setIsWaitingPreview] = React.useState(false);

  const settledPreviewSignatureRef = React.useRef<string>("");
  const expectedPreviousSignatureRef = React.useRef<string | null>(null);
  ...
  const previewSignature = React.useMemo(
    () => `${micrographPreviewUrl ?? ""}__${psdPreviewUrl ?? ""}`,
    [micrographPreviewUrl, psdPreviewUrl],
  );
  ...
  const beginPreviewRefresh = React.useCallback(() => {
    expectedPreviousSignatureRef.current =
      settledPreviewSignatureRef.current || previewSignature || null;
  ...
  React.useEffect(() => {
    ...
    const hasReadyPreview =
      Boolean(previewSignature && previewSignature !== "__") &&
      micrographLoaded &&
      psdLoaded;

    if (!hasReadyPreview) return;

    if (!isWaitingPreview) {
      settledPreviewSignatureRef.current = previewSignature;
      return;
    }

    const expectedPreviousSignature = expectedPreviousSignatureRef.current;
    const isNewPreview =
      !expectedPreviousSignature || previewSignature !== expectedPreviousSignature;

    if (isNewPreview) {
      settledPreviewSignatureRef.current = previewSignature;
      expectedPreviousSignatureRef.current = null;
  ...
  const hasAnyPreviewUrl = Boolean(micrographPreviewUrl || psdPreviewUrl);

  const showPreparingOverlay =
    open &&
    ((!hasAnyPreviewUrl) ||
      (!settledPreviewSignatureRef.current && (!micrographLoaded || !psdLoaded)));

  const showUpdatingOverlay = isWaitingPreview;
  const showLoadingOverlay = showPreparingOverlay || showUpdatingOverlay;

  const loadingLabel = showPreparingOverlay
    ? "Preparing wizard..."
    : "Updating preview...";
