...
const highlightText = (text: string, search: string) => {
  ...
  return parts.map((part, idx) =>
    regex.test(part) ? (
      <span key={idx} className="bg-yellow-300 dark:bg-yellow-600 font-bold">
        {part}
      </span>
    ) : (
      part
...
// --- Main tree ---
export const ProtocolsTree: React.FC<ProtocolsTreeProps> = ({ data, searchText = "" }) => {
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-2">
        {filteredData.map((node, idx) => (
          <ProtocolNodeItem key={idx} node={node} searchText={searchText} isRoot />
        ))}
      </div>
    </div>
  );
...
const ProtocolNodeItem: React.FC<{ node: ProtocolNode; searchText?: string; isRoot?: boolean }> = ({
  node,
  searchText,
  isRoot = false,
}) => {
  ...
  useEffect(() => {
    if (searchText && searchText.length > 0) {
      setExpanded(!!(hasChildren && (node.tag === "section" || node.tag === "protocol_group" || isRoot)));
    } else {
      ...
      {hasChildren && (
        <div className={`pl-5 border-l border-gray-300 dark:border-gray-600 ${expanded ? "block" : "hidden"}`}>
          {node.childs!.map((child, idx) => (
            <ProtocolNodeItem key={idx} node={child} searchText={searchText} />
          ))}
        </div>
      )}
    </div>
